/**Once upon a time, on a way through the old wild mountainous west,…
 ...
 Since this is thewildwest, with dreadful weather and not much water, it's important to save
...
function dirReduc(arr){
    for(let i =0; i< arr.length;){
        if(arr[i] === "NORTH" && arr.indexOf("SOUTH")!==-1 ){
            arr.splice(i,1)
            arr.splice(arr.indexOf("SOUTH"),1)
        }else if(arr[i] === "SOUTH" && arr.indexOf("NORTH")!==-1){
            arr.splice(i,1)
            arr.splice(arr.indexOf("NORTH"),1)
        }else if(arr[i] === "EAST" && arr.indexOf("WEST")!==-1 ){
            arr.splice(i,1)
            arr.splice(arr.indexOf("WEST"),1)
        }else if(arr[i] === "WEST" && arr.indexOf("EAST")!==-1 ){
            arr.splice(i,1)

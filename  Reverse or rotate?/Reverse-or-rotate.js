...
function revrot(str, sz) {
    if(str === ""|| sz > str.length|| sz<= 0){
        return ""
    }
    let result = ""
    let countOfLeft = 0
    let countOfRigth = 0
    let leftPart = str.slice(0,sz)
    let rigthPart = str.slice(sz,str.length)
    for(let i =0;i<leftPart.length;i++){
        if(+leftPart[i] % 2 === 1){
            countOfLeft++
        }
    }if(countOfLeft % 2 === 1){
        leftPart = leftPart.slice(1,leftPart.length) + leftPart[0]
    }else{
    ...
    for(let i =0;i<rigthPart.length;i++){
        ...
        rigthPart = rigthPart.slice(1,rigthPart.length) + rigthPart[0]
    }else{
        rigthPart = rigthPart.split("").reverse().join("")

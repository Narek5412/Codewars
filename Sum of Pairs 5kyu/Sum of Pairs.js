...
for (let i = 0; i < ints.length; i++) {
    for (let j = i+1; j < ints.length; j++) {
        if((ints[i]+ ints[j] === sum && smallestIndex > j) || (ints[i]+ ints[j] === sum && smallestIndex === undefined)) {
            firstIndex = i;
            smallestIndex = j;
        }
    }
}if(smallestIndex === undefined || firstIndex === undefined) {
    return undefined
    }
    console.log(ints.length)
return [ints[firstIndex], ints[smallestIndex]];
